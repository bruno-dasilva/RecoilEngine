...
Map populateMap(int n) {
	...
	Map map;
	if (USE_RESERVE) map.reserve(n * 3); // pre-size to avoid rehashing during measurement
	for (int i = 0; i < n; ++i)
...
Set populateSet(int n) {
	...
	Set set;
	if (USE_RESERVE) set.reserve(n * 3); // pre-size to avoid rehashing during measurement
	for (int i = 0; i < n; ++i)
...
void benchmarkMapMixed(Catch::Benchmark::Chronometer meter, int n, int findPct) {
	...
	if (USE_RESERVE) { for (auto& m : maps) m.reserve(n * 3); } // match populateMap's reserve
	meter.measure([&](int i) {
		auto& map = maps[i];
		uint64_t checksum = 0;
...
void benchmarkSetMixed(Catch::Benchmark::Chronometer meter, int n, int findPct) {
	...
	if (USE_RESERVE) { for (auto& s : sets) s.reserve(n * 3); } // match populateSet's reserve
	meter.measure([&](int i) {
		auto& set = sets[i];
		uint64_t checksum = 0;
		for (const auto& op : ops) {
...
{
	using Std    = std::unordered_map<int, int>;
	using Spring = spring::unordered_map<int, int>;
	using Unsync = spring::unsynced_map<int, int>;
...
{
	using Std    = std::unordered_map<std::string, std::string>;
	using Spring = spring::unordered_map<std::string, std::string>;
	using Unsync = spring::unsynced_map<std::string, std::string>;
...
TEST_CASE("Set<int>")
{
	using Std    = std::unordered_set<int>;
	using Spring = spring::unordered_set<int>;
	using Unsync = spring::unsynced_set<int>;
...
{
	using Std    = std::unordered_set<std::string>;
	using Spring = spring::unordered_set<std::string>;
	using Unsync = spring::unsynced_set<std::string>;
